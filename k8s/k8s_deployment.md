...
### Куда вставить этот блок?
Рекомендуемая структура раздела в вашем `README`:
```markdown
# Развертывание
## Docker Image
... (ваш текущий текст) ...
...
... (ваш текущий текст про HPA) ...
```

Если нужно адаптировать команды под конкретную среду (например, добавить `minikube tunnel` для `LoadBalancer` или автоматизировать всё через один `make deploy`), скажите — дополню.
